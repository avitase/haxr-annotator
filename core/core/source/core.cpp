...
int32_t write_root_attribute(hid_t file_id)
{
    ...
    const char *str = "modified_by_core";
    const size_t value_len = std::strlen(str);
    ...
    const char *attr_name = "haxr_poc";
    const hid_t attr_id = (H5Aexists(root_id, attr_name) > 0)
        ? H5Aopen(root_id, attr_name, H5P_DEFAULT)
        : H5Acreate2(
              root_id, attr_name, type_id, space_id, H5P_DEFAULT, H5P_DEFAULT);
    ...
    const herr_t write_status = H5Awrite(attr_id, type_id, str);
...
extern "C"
{
int32_t haxr_tag_hdf5_image(uint8_t *input_data, size_t input_size)
{
    if (input_data == nullptr) {
        return -10;
    }

    const std::unique_ptr<uint8_t, decltype(&std::free)> input_guard(
        input_data, &std::free);
    if (input_size == 0) {
        return -10;
    }
    ...
    if (H5Pset_file_image(fapl_id, input_data, input_size) < 0)
    {
        H5Pclose(fapl_id);
        return -13;
    }
    ...
    const hssize_t written =
        H5Fget_file_image(file_id, buffer, static_cast<size_t>(image_size));
    H5Fclose(file_id);
    if (written <= 0) {
        delete[] buffer;
        return -17;
